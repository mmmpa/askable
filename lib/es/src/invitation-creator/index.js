...
class Context extends ContextComponent {
  ...
  submit (params) {
    ...
    API.invite(this.setBase(params))
      .then((result)=> {
        this.setState({ result, errors: {}, state: Condition.Success });
      })
      .catch((result)=> {
        let { errors } = result;
...
class Component extends ComponentComponent {
  ...
  componentWillUpdate (props) {
    if (this.props.state !== Condition.Success && props.state === Condition.Success) {
      this.setState({ login: '' })
    }
  }
    ...
    switch (this.props.state) {
      case Condition.Success:
        return <div className="invitation message-area">
          <p className="com success-message">Invited the user.</p>
        </div>;
  ...
  render () {
    ...
    return <section className="invitation body">
      <div className="invitation input-area">
        ...
        <SubmitButton {...{
          state, icon: "envelope-o", text: "Invite", className: 'submit',
